...
for snr in s:
    ...
    mat = h5py.File('D:/project_new/lsdnn/train_norm/Dataset_{}.mat'.format(snr), 'r')
    ...
    for i in range(scalerx.mean_.shape[0]):
        a[i] = scalerx.mean_[i]
    np.savetxt('D:/project_new/lsdnn/train_norm/DNN{}/mean.dat'.format(snr), a, fmt='%f', newline=',')
    for i in range(scalerx.mean_.shape[0]):
        b[i] = math.sqrt(scalerx.var_[i])
    np.savetxt('D:/project_new/lsdnn/train_norm/DNN{}/std.dat'.format(snr), b, fmt='%f', newline=',')
    for i in range(scalery.mean_.shape[0]):
        a[i] = scalery.mean_[i]
    np.savetxt('D:/project_new/lsdnn/train_norm/DNN{}/mean_o.dat'.format(snr), a, fmt='%f', newline=',')
    for i in range(scalery.mean_.shape[0]):
        b[i] = math.sqrt(scalery.var_[i])
    np.savetxt('D:/project_new/lsdnn/train_norm/DNN{}/std_o.dat'.format(snr), b, fmt='%f', newline=',')
    ...
    model_path = 'D:/project_new/lsdnn/train_norm/LS_DNN_{}.h5'.format(snr)
